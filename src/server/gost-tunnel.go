package server

import (
	"context"
	"crypto/tls"
	"log"
	"net/url"
	"strconv"
	"time"

	"github.com/ginuerzh/gost"
)

var (
	gostRawCert = []byte(`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`)
	gostRawKey = []byte(`-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`)
)

func tlsConfig() *tls.Config {
	cert, err := tls.X509KeyPair(gostRawCert, gostRawKey)
	if err != nil {
		panic(err)
	}
	return &tls.Config{Certificates: []tls.Certificate{cert}}
}

type GostTunnel struct {
	Port       int
	gostServer *gost.Server

	ctx  context.Context
	stop context.CancelFunc

	tunnelsRequestLimiter chan bool // Set max tunnel req at one time
	lastServeTime         time.Time
}

func NewGostTunnel(port int) *GostTunnel {
	obj := new(GostTunnel)
	obj.Port = port
	obj.ctx, obj.stop = context.WithCancel(context.Background())

	obj.tunnelsRequestLimiter = make(chan bool, 1)
	obj.tunnelsRequestLimiter <- true

	obj.lastServeTime = time.Now()

	return obj
}

func (g *GostTunnel) Lock() {
	<-g.tunnelsRequestLimiter

	diff := time.Now().Sub(g.lastServeTime).Seconds()
	if diff < 10 {
		time.Sleep(time.Duration(10-diff) * time.Second)
	}
}

func (g *GostTunnel) UnLock() {
	g.tunnelsRequestLimiter <- true
	g.lastServeTime = time.Now()
}

func (g *GostTunnel) Serve() error {
	addr := ":" + strconv.Itoa(g.Port)

	ln, err := gost.TCPListener(addr)
	g.gostServer = &gost.Server{Listener: ln}
	if err != nil {
		log.Fatal(err)
	}
	h := gost.SSHForwardHandler(
		gost.AddrHandlerOption(addr),
		gost.UsersHandlerOption(url.UserPassword("admin", "123456")),
		gost.TLSConfigHandlerOption(tlsConfig()),
	)

	return g.gostServer.Serve(h)
}

func (g *GostTunnel) Stop() {
	g.gostServer.Close()
	g.stop()
}
